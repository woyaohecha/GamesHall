...
export class Util {
    ...
    public static getParamValueFromLocationHref(code: string) {
    ...
    public static _stringToArray(string: string) {
        ...
        let rsRegional = '(?:\\ud83c[\\udde6-\\uddff]){2}';
        let rsSurrPair = '[\\ud800-\\udbff][\\udc00-\\udfff]';
        let rsOptJoin = '(?:' + rsZWJ + '(?:' + [rsNonAstral, rsRegional, rsSurrPair].join('|') + ')' + rsOptlet + reOptMod + ')*';
        let rsSeq = rsOptlet + reOptMod + rsOptJoin;
 ...
 */
    ...
    public static checkIsLowPhone(): Boolean {
        ...
        if (window.wx) {
            ...
            if (isIOS) {
                ...
                const highPhoneType = ['iPhone11,2', 'iPhone11,4', 'iPhone11,6', 'iPhone11,8', 'iPhone12,1', 'iPhone12,3', 'iPhone12,5', 'iPhone12,8', "iPhone 13"];
    ...
    public static numToArray(num: number) {
        var str = num + "";
        var resultArr = [];
     ...
     * @param {string} t 
     */
    public static getTimeAtom = (t: string) => {
        if (!t) {
            return null
        }
